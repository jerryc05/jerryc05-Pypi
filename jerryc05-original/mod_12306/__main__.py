def main(args: list = None):
	...
	date = args[2]
	while not _len(date) == 10 or not \
					_int(date[:4]) > 2018 or not \
					0 < _int(date[5:7]) < 13 or not \
					0 < _int(date[8:]) < 32:
		if _len(date) == 8:
			date = f'{date[:4]}-{date[4:6]}-{date[6:8]}'
		else:
			date = ipt(f'Date "{date}" invalid, retry: ')
			...
			while not station or not station[0][0]:
				...
				for index, item in enumerate(station):
					prt(
						...
						f'| {item[1]:{12 - _len(item[1]) + item[1].count(" ")}} |')
				prt('+-----+--------------------+------+--------------+')

				if not station[0][0]:
					station = ()
					city = ipt(f'City name "{city}" not found, retry: ')

			__index = _int(ipt('Index number: ')) - 1
			while not 0 <= __index < _len(station):
				__index = _int(ipt('Index number invalid, retry: ')) - 1

			__station: tpl = station[__index]
			prt(
				'Chosen station name:\n'
				'+-----+--------------------+------+--------------+\n'
				f'| {__index + 1:3} | {__station[3]:18} | {__station[2]:4} '
				f'| {__station[1]:{12 - _len(__station[1]) + __station[1].count(" ")}} |\n'
				'+-----+--------------------+------+--------------+\n\n'
			)
	...
	if 1:  # Network accessing
		...
		with u_req.urlopen(
			...
			if b'<' in r_bytes[:4]:
				raise SysErr(
					f'Invalid date, {date} may be some date in the past.'
				)
			try:
				train_data: tpl = json.loads(r_bytes)['data']['result']
			except Exception as e:
				raise SysErr(
					f'Internal process error, please contact support. Detail: {e}'
				)
